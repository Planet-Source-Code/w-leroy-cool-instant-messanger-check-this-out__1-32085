...
Private Function Change_PW(ByVal Msg As Variant)
    ...
    Temp = Left(Msg, 7) 'This is all the digits in the string
    
    nTemp = Right(Msg, 2) 'Now read out the needed portion
    
    If Left(nTemp, 1) = "0" Then
        
        Name_Len = Right(nTemp) 'Store it now if it's 1 digit len
    
    Else
        
        Name_Len = Int(nTemp) 'Store it here if its a two digit number
    
    End If
    
    uName = Right(Msg, Name_Len) 'Now store the users name
    
    'Now this is your old password being stored here " eHem old PassWord "
    Temp = Right(Msg, oLen + Name_Len) 'Store old pw
    
    oPW = Left(Temp, oLen) 'Store old pw
    
    'Getting the portion we need
    Temp = Trim(Right(Msg, oLen + nLen + Name_Len))
    ...
    'Search database for uName and change PW
